...
To produce guidance for following a path and avoiding obstacles, a repulsive obstacle vector field $\overrightarrow{V}_{||O||}$ needs to be constructed and summed with the path following guidance $\overrightarrow{V}_{||P||}$. The repulsive vector field is multiplied 
The sum of the two guidances is represented by $\overrightarrow{V}_g$ and is shown in Equation \ref{eq:totalGuidance}


\begin{equation}\label{eq:totalGuidance}
...
A circular avoidance vector field will now be constructed in a way similar to that of the path following field in the previous section. What differentiates the obstacle field from the path following field is that the individual convergence and circulation components are normalized prior to normalizing the summed obstacle field $\overrightarrow{V}_o$. Additionally, negative convergence weights will be used to produce vectors that diverge away from the path. The obstacle vector field is constructed using the normalized component Equation \ref{eq:obstComponents} with obstacle convergence and circulation weights $G_o$ and $H_o$ respectively.
...
\end{equation}
 ...
 Note that the magnitude of the vectors in $\overrightarrow{V}_{conv}$ and $\overrightarrow{V}_{circ}$ change in length as they converge to the path. Normalizing the component vectors in Equation \ref{eq:obstComponents} constraints the vectors to a length of unity. The contribution of each component is modified by varying the scalars $G_o$ and $H_o$.  \\
...
\overrightarrow{0} = \overrightarrow{V}_{path} +P\overrightarrow{V}_o
\end{equation}

\begin{equation}
\label{eq:zeroVectorCondition2}
\overrightarrow{V}_{path}=-P\overrightarrow{V_o}
\end{equation}
